...
function atualizarPagina() {
  campoTexto.value = paginas[paginaAtual];
  infoData.textContent = new Date().toLocaleDateString('pt-BR');
...
campoTexto.addEventListener('input', () => {
  paginas[paginaAtual] = campoTexto.value;
  salvarNoNavegador();
});
...
document.getElementById('verTudo').addEventListener('click', () => {
  todasNotas.innerHTML = paginas.map((nota, i) => 
    `<p><strong>Página ${i + 1}:</strong> ${nota || '<em>Vazia</em>'}</p>`
  ).join('');
